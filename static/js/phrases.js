(function () {
  'use strict';
  const { api, swr, swrInvalidate, toast, esc, confirmSheet, actionSheet, openSheet, closeSheet, attachLongPress, bindDraft, isComposing } = window.AppUI;
  let items = [], filter = '', q = '';
  async function load() { await swr(`/api/phrases?learned=${filter}&q=${encodeURIComponent(q)}`, (d) => { items = d.items; render(d.total); }); api('/api/stats/badge').then((b) => document.getElementById('cntDue').textContent = b.phrase_due || '').catch(() => {}); }
  function render(total) {
    ...
    document.getElementById('list').innerHTML = items.map((p) => `
      <div class="phrase-item glass rounded-2xl p-4 ${p.learned ? 'opacity-70' : ''}" data-id="${p.id}">
        ...
        <div class="flex items-center gap-1 mt-2 text-[11px]">
          <button class="speak hit px-2 py-2 rounded-lg text-white/50 hover:text-white" data-text="${esc(p.phrase)}"><i class="fas fa-volume-high mr-1"></i>듣기</button>
          <button class="learn hit px-2 py-2 rounded-lg ${p.learned ? 'text-emerald-300' : 'text-white/50 hover:text-white'}" data-id="${p.id}" data-learned="${p.learned}"><i class="fas ${p.learned ? 'fa-circle-check' : 'fa-circle'} mr-1"></i>${p.learned ? '익힘' : '익혔어요'}</button>
    ...
    document.querySelectorAll('.learn').forEach((b) => b.addEventListener('click', async () => {
      // 낙관적 UI: 바로 바꾸고 실패하면 되돌린다
      const v = b.dataset.learned !== 'true', it = items.find((x) => x.id === b.dataset.id); if (it) it.learned = v; render(total);
      try { await api(`/api/phrases/${b.dataset.id}`, { method: 'PATCH', body: { learned: v } }); swrInvalidate('/api/phrases'); load(); }
      catch (e) { if (it) it.learned = !v; render(total); toast(e.message, false); }
    }));
  ...
  function menu(p) {
    ...
    actionSheet({ title: p.phrase, subtitle: p.meaning_ko, actions: [
      ...
      { icon: p.learned ? 'fa-rotate' : 'fa-circle-check', label: p.learned ? '다시 복습하기' : '익혔어요', onClick: async () => { await api(`/api/phrases/${p.id}`, { method: 'PATCH', body: { learned: !p.learned } }); swrInvalidate('/api/phrases'); load(); } },
      'sep',
      { icon: 'fa-trash', label: '삭제', danger: true, onClick: async () => { if (await confirmSheet({ title: '표현 삭제', message: `"${p.phrase}" 을(를) 삭제할까요?`, confirmLabel: '삭제', danger: true })) { document.querySelector(`.phrase-item[data-id="${p.id}"]`)?.remove(); await api(`/api/phrases/${p.id}`, { method: 'DELETE' }); swrInvalidate('/api/phrases'); toast('삭제했어요'); load(); } } },
    ] });
  }
  ...
  document.getElementById('addForm').addEventListener('submit', async (e) => { e.preventDefault(); try { await api('/api/phrases', { method: 'POST', body: { phrase: addPhrase.value, meaning_ko: addMeaning.value } }); addPhrase.value = ''; addMeaning.value = ''; clearA(); clearB(); swrInvalidate('/api/phrases'); toast('추가했어요'); load(); } catch (err) { toast(err.message, false); } });
  ...
  document.getElementById('quizBtn').addEventListener('click', async () => {
    ...
    openSheet('<div class="sheet-head"><div><h3><i class="fas fa-graduation-cap text-purple-300 mr-2"></i>복습 퀴즈</h3></div><button type="button" class="sheet-close hit" aria-label="닫기">&times;</button></div><div id="quizBody" class="pb-2"></div>', (root) => { root.querySelector('.sheet-close').addEventListener('click', () => closeSheet()); showCard(root.querySelector('#quizBody')); }, () => { swrInvalidate('/api/phrases'); load(); });
  });
