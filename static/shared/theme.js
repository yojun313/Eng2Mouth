...
(function () {
  ...
  function applyMode(mode) {
    ...
    set(MODE_KEY, mode);
    try {
      window.dispatchEvent(new CustomEvent('e2m-theme-change', { detail: { mode: mode, style: currentStyle() } }));
    } catch (e) { /* noop */ }
  }
  ...
  function buildModal() {
    ...
    overlay.id = 'e2mThemeOverlay';
    overlay.className = 'e2m-theme-overlay';
    overlay.hidden = true;

    var optionsHtml = THEMES.map(function (t) {
      return (
        '<button type="button" class="e2m-theme-option" data-theme="' + t.id + '">'
        + '<span class="e2m-theme-swatch e2m-swatch-' + t.id + '"></span>'
        + '<span class="e2m-theme-label"><span class="e2m-theme-check">✓</span>' + t.label + '</span>'
        + '</button>'
      );
    }).join('');

    overlay.innerHTML =
      '<div class="e2m-theme-modal" role="dialog" aria-modal="true" aria-label="테마 설정">'
      + '<div class="e2m-theme-head">'
      + '<div><h3>테마 설정</h3><p>원하는 테마를 골라보세요. 모든 페이지에 동일하게 적용됩니다.</p></div>'
      + '<button type="button" class="e2m-theme-close" aria-label="닫기">&times;</button>'
      + '</div>'
      + '<div class="e2m-theme-body">' + optionsHtml + '</div>'
      + '<div class="e2m-theme-mode-row"><span>다크 모드</span>'
      + '<button type="button" class="e2m-mode-switch" id="e2mModeSwitch" aria-label="다크 모드 전환"></button></div>'
      + '</div>';
  ...
  function markSelected(overlay) {
    ...
    overlay.querySelectorAll('.e2m-theme-option').forEach(function (btn) {
      btn.classList.toggle('selected', btn.getAttribute('data-theme') === active);
    });
    overlay.querySelector('#e2mModeSwitch').classList.toggle('on', currentMode() === 'dark');
  }
  ...
  function init() {
    ...
    overlay.querySelector('.e2m-theme-close').addEventListener('click', close);
    overlay.querySelectorAll('.e2m-theme-option').forEach(function (opt) {
    ...
    overlay.querySelector('#e2mModeSwitch').addEventListener('click', function () {
